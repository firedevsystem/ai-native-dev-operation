---
title: "第4章：Phase 3〜5 ── 設計とプロトタイプの反復ループ"
emoji: "🏗️"
type: "tech"
...
> **AIネイティブ開発 連載目次**
> 序章 / 第1章 / 第2章 / 第3章 / **第4章（この記事）** / 第5章 / 第6章 / 第7章
...
:::details API設計の癒着パターン
**パターンA：責務の混同。** 1つのAPIが「一覧取得」と「集計」を同時にやっている。分離すべきだ。

例えば、`/api/projects` が一覧を返しつつ、各プロジェクトの工数合計も計算して返すケース。一覧取得と工数集計は別の責務であり、`/api/projects`（一覧）と `/api/projects/{id}/hours`（工数集計）に分離すべきだ。

**パターンB：レスポンスの肥大化。** ユーザー情報を返すAPIに、そのユーザーの注文履歴も所属部署も全部含めている。ユーザー情報、注文履歴、所属部署はそれぞれ別のAPIとして切るべきだ。

例えば、`/api/users/{id}` のレスポンスに `orders: [...]`（注文履歴配列）や `departments: [...]`（所属部署配列）が含まれるケース。`/api/users/{id}/orders` や `/api/users/{id}/departments` として独立させる。
:::
...
**← 前の記事：** [第3章 Phase 0〜2：顧客の真の課題に到達するまで妥協しない](https://zenn.dev/firedev/articles/ai-native-dev-03-phase0-2)
**→ 次の記事：** [第5章 Phase 6〜8：フィードバックからフルスケールへ](https://zenn.dev/firedev/articles/ai-native-dev-05-phase6-8)
